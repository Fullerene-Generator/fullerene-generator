...
LSignatureState::LSignatureState(const dual_fullerene& G, const LCandidate& c)
    : graph_(&G),
    candidate_(&c),
    bfs_front_(0),
    finished_(false)
...
void LSignatureState::extend_step() {
    if (finished_) {
        return;
    }
...
bool LSignatureState::finished() const {
    return finished_;
}

const std::vector<int>& LSignatureState::signature() const {
    return signature_;
}
